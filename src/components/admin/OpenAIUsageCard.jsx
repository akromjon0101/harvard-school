...
export default function OpenAIUsageCard() {
    ...
    const fetchUsage = useCallback(async () => {
        ...
        setError(null)
        try {
            const data = await api('/openai-usage')
            setData(data)
            setLastUpdated(new Date())
        } catch (err) {
            setError(err.message || 'Unable to fetch usage data')
            setData(null)
        } finally {
            setLoading(false)
        }
    ...
    const saveBudget = async () => {
        ...
        setBudgetError('')
        try {
            await api('/openai-usage/budget', 'PUT', { budget: val })
            setEditingBudget(false)
            fetchUsage()
        } catch (err) {
            setBudgetError(err.message || 'Xato yuz berdi')
        } finally {
